...
static const int NUM_THREADS = 2;		// Cantidad de hilos
...
int llamadasNode=0;
int sizeNewEdge;
int blockedThreads = 0;		// Contador para los hilos en espera de trabajo
bool finished;

// Variables globales de hilos 

pthread_cond_t work_complete_cv;
pthread_cond_t blocked_threshold_cv;
pthread_mutex_t queue_mutex;
pthread_mutex_t cost_mutex;
pthread_mutex_t count_blocked_mutex;
pthread_mutex_t node_count_mutex;

//----------------------------------------------//

void *checkBlockedThreads(void *arg);
...
public:
	...
	int assignPoint (Point p, int edgeIndex,vector<Point> &newEdge) 
	{
		...
		while (edgeIndex < sizeNewEdge && constraint[(int) abs((double)pt.getX())-1][(int) abs((double)pt.getY())-1]!= 0) 
		{
			edgeIndex++;
		...
		{
			if (pt.getX() < 0) 
				assignConstraint(-1,(int) abs((double)pt.getX())-1,(int) abs((double)pt.getY())-1);
			else 
	...
	~Node()
	{
		...
		constraint=0;
		restrict_conect=0;
		ciudadesAlcanzables=0;
		ciudadesNoAlcanzables=0;
		b = 0;
	}
};
...
public:
	bool operator()	(Node *a, Node *b)
	{
		if(a->getLowerBound() == b->getLowerBound())	
			...
			return a->getLowerBound() > b->getLowerBound();	// Si son distintos, tiene prioridad el de menor lb
	}
};
...
public:	
	...
	{
		int i,col,row,temp=0,bestCost;
		Point pt;
		Node *root;
		pthread_t threads[NUM_THREADS];
		pthread_t t;	// Comprueba si se termina el trabajo de los hilos
		...
		root->setIndex(-1);
		q.push(root);	// Cambiado, ahora se apila aca en vez de branch and bound
		
		sizeNewEdge = newEdge.size();	// Cantidad de arcos
		
		// Inicializacion de mutex y variables de condicion
		pthread_mutex_init(&queue_mutex,NULL);
		pthread_mutex_init(&cost_mutex,NULL);
//		pthread_mutex_init(&node_count_mutex,NULL);		
		pthread_cond_init(&work_complete_cv, NULL);
		pthread_cond_init(&blocked_threshold_cv, NULL);
		
		// Inicializacion de hilos
		pthread_create(&t, NULL, checkBlockedThreads, NULL);
		
		for(i = 0; i < NUM_THREADS; i++)
			pthread_create(&threads[i], NULL, processNode, this);
		...
		// Destruccion de los mutex y variables de condicion
		pthread_mutex_destroy(&queue_mutex);
		pthread_mutex_destroy(&cost_mutex);
//		pthread_mutex_destroy(&node_count_mutex);
		pthread_cond_destroy(&work_complete_cv);
		pthread_cond_destroy(&blocked_threshold_cv);
...
void *checkBlockedThreads(void *arg)
{
	...
	while(blockedThreads < NUM_THREADS)
	{
		pthread_cond_wait(&blocked_threshold_cv, &queue_mutex);
	}
	
	finished = true;							// Indicamos que se ha finalizado el trabajo
	pthread_cond_broadcast(&work_complete_cv);	// Despertamos todos hilos que esperan por trabajo, para que finalizen
...
void *processNode(void *arg)
{
	TSP *tsp;	// Usado para hacer el casting con arg
	Point p;
	Node *leftChild, *rightChild;
	int leftEdgeIndex = 0, edgeIndex,rightEdgeIndex = 0;
	Node *pivot;
	tsp = static_cast<TSP*>(arg);

	while(true)	
	{
		pthread_mutex_lock(&queue_mutex);
			
		while(q.empty())	// Si no hay nodos
		{
			blockedThreads++;
			if(blockedThreads == NUM_THREADS)			// Si se bloquean todo los hilos
			{
				pthread_cond_signal(&blocked_threshold_cv);	// Hay que terminar todos los hilos 
				pthread_mutex_unlock(&queue_mutex);
				pthread_exit(NULL);
			}
			
			//while(q.empty())
			pthread_cond_wait(&work_complete_cv, &queue_mutex);		// Esperamos un nuevo nodo
			
			if(finished)	// Termina el trabajo, ya que no llegaran mas nodos
			{
				pthread_mutex_unlock(&queue_mutex);
				pthread_exit(NULL);	
			}
		...
		if (pivot != 0 && abs((double)edgeIndex) < sizeNewEdge)  //Mientras pivot no sea nulo y no haya excedido el tamaño del vector de puntos
		{
			if(pivot->isTour()) //Si pivot es tour
			{	
				pivot->setTour(tsp->c);//Almacenar el tour
				
				pthread_mutex_lock(&cost_mutex);
				if (pivot->getTourCost() < CostoTour) // Si es menor al tour que ya tenemos 
				{
					delete tsp->bestNode;//Eliminar el nodo con el tour anterior
					CostoTour = pivot->getTourCost();//Almacenar el nuevo costo 
					tsp->bestNode = pivot; // Apuntar al nuevo nodo contenedor del tour
				}
			...
			else //Sino es tour
			{
				...
				if (pivot->getLowerBound() < 2 * CostoTour) //Si es menor al costo del tour que tenemos actualmente
				{
					...
					if(edgeIndex < sizeNewEdge) //Si no excedemos la cantidad de posiciones de newEdge
					{
						...
						{	
							p = (Point) tsp->newEdge[leftEdgeIndex + 1];
							rightEdgeIndex = rightChild->assignPoint(p, leftEdgeIndex +1,tsp->newEdge);
							rightChild->setIndex(leftEdgeIndex +1);
							rightChild->addDisallowedEdges();		
							rightChild->addRequiredEdges();			
							rightChild->computeLowerBound(tsp->c);	
							...
							if (leftChild != 0 && rightChild == 0) 
							{
								pthread_mutex_lock(&queue_mutex);
								q.push(leftChild);
								pthread_cond_signal(&work_complete_cv);
								pthread_mutex_unlock(&queue_mutex);
							}
					...
					else// El index izquierdo excedio el tamaño del vector de puntos, elimino pivot y leftchild
					{
						delete leftChild;
						leftChild = 0;
						delete pivot;
